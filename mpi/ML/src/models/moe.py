"""Mixture of Experts (MoE) — комбинирование нескольких реконструкторов.

## Идея

Разные методы реконструкции MPI имеют разные сильные стороны:
  • **Tikhonov** хорошо подавляет шум на гладких участках, но размывает
    границы;
  • **Kaczmarz/ART** даёт точечные ответы, но осциллирует на сложных
    структурах;
  • **Chae-Single FC** аппроксимирует обращение SM полиномами Чебышёва
    (хорошо на гладких выпуклых фантомах);
  • **Chae-Multi FC** с hidden-слоем точнее ловит non-linear эффекты
    Ланжевена (помогает на мелких частицах);
  • **CNN (U-Net)** даёт чёткие границы, но плохо работает с очень
    тонкими структурами.

Mixture of Experts комбинирует выходы нескольких таких моделей через
обучаемую gating-сеть: для каждой точки изображения система решает,
какому эксперту в большей степени доверять. Результат — комбинация
сильных сторон без явного выбора одного «лучшего» алгоритма.

## Архитектура

  Input: measurement u (одно измерение со сканера)
  ↓
  Эксперты (заморожены): expert_1(u), expert_2(u), ..., expert_K(u)
  ↓ K реконструкций, каждая (H, W)
  Stack: tensor shape (K, H, W)
  ↓
  Gating-сеть g_φ: выдаёт K весов на каждый пиксель
  ↓ shape (K, H, W), softmax по K → сумма по K = 1
  Output: Σ_k g_φ[k, :, :] · expert_k(u)

Обучается ТОЛЬКО gating-сеть (на парах u_train, c_true). Эксперты
зафиксированы — их выходы прекомпилируются один раз и кешируются.

## Три режима комбинирования (mode=)

  • **'mean'**     — простое усреднение, без обучаемых параметров.
                     Baseline для сравнения.

  • **'scalar'**   — один скалярный вес α_k на эксперта, общий для всех
                     пикселей. Softmax(α) даёт распределение «доверия» по
                     экспертам. Глобальное смешивание; реконструкция —
                     ровно одна и та же взвешенная сумма везде.

  • **'spatial'**  — per-pixel gating: маленькая CNN смотрит на стек
                     выходов всех экспертов и предсказывает K весов в
                     каждом пикселе (softmax по k). Самый выразительный
                     режим: один эксперт может «доминировать» в одной
                     части изображения, другой — в другой.

## Эксперты как callable

Эксперты передаются как словарь {name: callable}, где callable:
  • принимает один аргумент — измерение в формате методов проекта;
  • возвращает 2D numpy-массив формы image_shape (нормированный или нет).

Это позволяет вкладывать любые методы — Tikhonov-реконструктор,
обученную CNN, классы PMCNet — без изменения их интерфейса.

## Когда работает плохо

Если все эксперты дают плохие реконструкции на данных, MoE тоже не
поможет (gating не может извлечь хорошее из плохого). Это важный
practical обходчик: на physical-пути данных, где CNN/Chae обучены
на SM-данных, их выходы низкого качества → MoE-комбинация тоже плохая.
"""
