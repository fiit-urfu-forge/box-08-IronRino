"""Загрузчик OpenMPI Dataset для real-data валидации методов реконструкции.

## Что такое OpenMPI Dataset

OpenMPI Dataset — публичный набор калибровочных и измерительных данных
MPI-сканеров в формате MDF (Magnetic particle imaging Data Format).
Это HDF5-файлы со специфической структурой групп:
  • acquisition/    — параметры сканера (drive_field, gradient, etc.);
  • measurement/    — собственно измерения с раскладкой data/r, data/i
                      для real и imag частей;
  • calibration/    — параметры калибровочного сканирования;
  • scanner/        — метаданные железа.

## Зачем нужен в проекте

Все нейросетевые модели обучаются на СИНТЕТИКЕ. Чтобы оценить, насколько
они работают на РЕАЛЬНЫХ измерениях, нужен независимый источник реальных
данных. OpenMPI как раз такой: измерения с реальных сканеров в открытом
доступе, без необходимости иметь свой сканер.

## Что здесь есть

  • `OpenMPIDataManager`: высокоуровневый загрузчик. Сканирует локальную
    папку, парсит структуру MDF, извлекает измерения и калибровочные SM.

  • `OpenMPIDataset`: torch.utils.data.Dataset wrapper, для удобной
    подачи данных в DataLoader (если кому-то нужно дообучить модель на
    OpenMPI данных).

  • `resolve_openmpi_dir()`: поиск папки OpenMPIData в типичных
    расположениях. Возвращает первый существующий путь.

## Важно

Данные НЕ скачиваются из сети — используются только локальные .mdf
файлы из ChineseData/OpenMPIData/. Если папки нет, валидация
пропускается без ошибки (флаг `validate_openmpi=False` по умолчанию
в run.py).

Подключение к pipeline:
  • Активируется через `validate_openmpi=True` в `run.py`;
  • Запускается в самом конце `run_pipeline()` после основного сравнения
    на синтетических фантомах.
"""
