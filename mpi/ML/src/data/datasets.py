"""Загрузка реальных калибровочных данных сканера + Torch Dataset обёртки.

## Что здесь есть

  • **MPIDatasetGenerator** — загружает реальную системную матрицу
    сканера из калибровочного H5-файла (BeihangUniversityData/
    SystemMatrix.h5) и параметры FOV.

    Системная матрица — это таблица (M × N) комплексных чисел, где
    строка m содержит спектральный отклик системы на единичную
    концентрацию в каждом из N вокселей. Получается путём
    последовательного помещения точечного фантома в каждый воксель
    и записи сигнала.

    Для типичной MPI-конфигурации 51×51 вокселя (N=2601), 1275 гармоник
    на катушку × 2 катушки (M=2550): матрица содержит 6.6M комплексных
    чисел (~52 МБ во float32). Не очень большая, помещается в память.

  • **MPIDataset** — обёртка `torch.utils.data.Dataset` для подачи пар
    `(measurement, image)` в DataLoader. Используется в supervised-
    тренировке моделей (CNN, MoDL и т. д.).

## Почему отдельный от simulators.py

`simulators.py` создаёт СИНТЕТИЧЕСКИЕ данные с НУЛЯ (через физику или
Чебышёва). `datasets.py` загружает РЕАЛЬНЫЕ данные сканера. Два
независимых источника:
  • Синтетика для обучения (быстрая генерация, известный ground truth);
  • Реальная SM для генерации измерений через S·c (с реалистичной
    калибровкой сканера) и валидации.

В pipeline.py обычно используются ОБЕ: синтетический ground-truth
изображения генерируются через PhantomGenerator, а измерения для них
синтезируются через реальную SM сканера — это даёт «реалистичные»
обучающие данные без необходимости делать тысячи реальных измерений.
"""
